...
[原文链接（日语）](https://w.atwiki.jp/utaou/pages/64.html) 作者：[UTAU ユーザー互助会@ ウィキ](https://w.atwiki.jp/utaou/) 翻译：[SuibianP](https://github.com/SuibianP)
...
- [UTAU插件标准](#utau插件标准)
  ...
  - [基本信息](#基本信息)
    - [插件的形式](#插件的形式)
    - [`plugin.txt`的格式](#plugintxt的格式)
    - [`install.txt`的格式](#installtxt的格式)
    - [文件夹构成](#文件夹构成)
  - [数据输入输出方法](#数据输入输出方法)
    - [输入](#输入)
    - [输出](#输出)
  - [数据格式](#数据格式)
    ...
    - [条目](#条目)
      - [一定存在的条目](#一定存在的条目)
        - [`Length`　音符的长度](#length音符的长度)
        - [`Lyric`　词](#lyric词)
        - [`NoteNum`　音阶编号](#notenum音阶编号)
        - [`PreUtterance`　先行发声](#preutterance先行发声)
      - [有可能被省略的条目](#有可能被省略的条目)
        - [`VoiceOverlap`　重叠部分](#voiceoverlap重叠部分)
        - [Intensity　音的强度](#intensity音的强度)
        - [`Moduration`　移调](#moduration移调)
        - [`StartPoint`　STP](#startpointstp)
        - [`Envelope`　包络](#envelope包络)
        - [`Tempo`　曲速](#tempo曲速)
        - [`Velocity`　子音速度](#velocity子音速度)
        - [`Label`　标签](#label标签)
        - [`$direct`　直接输出](#direct直接输出)
        - [`$patch`　wav文件直接输出](#patchwav文件直接输出)
        - [`$region`　选择范围的开始](#region选择范围的开始)
        - [`$region_end`　选择范围的终止](#region_end选择范围的终止)
      - [只读条目](#只读条目)
        - [`@preuttr`　自动调整完成的先行发声](#preuttr自动调整完成的先行发声)
        - [`@overlap`　自动调整完成的重叠部分](#overlap自动调整完成的重叠部分)
        - [`@stpoint`　自动调整完成的STP](#stpoint自动调整完成的stp)
        - [`@filename`　渲染使用文件名](#filename渲染使用文件名)
        - [`@alias`　别名](#alias别名)
        - [`@cache`　缓存文件名](#cache缓存文件名)
      - [音调（模式1）](#音调模式1)
        - [`PBType`　滑音类型](#pbtype滑音类型)
        - [`Piches`　音调数列](#piches音调数列)
        - [`PBStart`　音调数列开始位置](#pbstart音调数列开始位置)
      - [音调（模式2）](#音调模式2)
        - [`PBS`　音高曲线最初点](#pbs音高曲线最初点)
        - [`PBW`　音高点间隔](#pbw音高点间隔)
        - [`PBY`　除最初点外的音高点移动值](#pby除最初点外的音高点移动值)
        - [`PBM`　音高曲线的形状](#pbm音高曲线的形状)
        - [`VBR`　颤音](#vbr颤音)
      - [Flags](#flags)
        - [`b`　不通过共振峰修正的BRE](#b不通过共振峰修正的bre)
        - [`B`　BRE](#bbre)
        - [`c`　低通滤波器（共振峰修正前）](#c低通滤波器共振峰修正前)
        - [`C`　低通滤波器1](#c低通滤波器1)
        - [`D`　低通滤波器2](#d低通滤波器2)
        - [`E`　低通滤波器3](#e低通滤波器3)
        - [`F`　共振峰修正适用频率范围（音高基准）](#f共振峰修正适用频率范围音高基准)
        - [`g`　简易性别参数](#g简易性别参数)
        - [`G`　频率表再生成](#g频率表再生成)
        - [`h`　低通滤波器4（BRE以外）](#h低通滤波器4bre以外)
        - [`H`　低通滤波器4](#h低通滤波器4)
        - [`L`　共振峰修正适用频率范围（频率基准）](#l共振峰修正适用频率范围频率基准)
        - [`N`　无共振峰滤波器](#n无共振峰滤波器)
        - [`P`　波峰压缩器强度](#p波峰压缩器强度)
        - [`R`　TIPS引擎用参数文件再生成](#rtips引擎用参数文件再生成)
        - [`t`　以10音分为单位移动音程](#t以10音分为单位移动音程)
        - [`T`　以文本输出频率表](#t以文本输出频率表)
        - [`W`　机器人声生成](#w机器人声生成)
        - [`Y`　伸缩范围的BRE比例](#y伸缩范围的bre比例)
        - [`/`　切换为高速化批处理用引擎](#切换为高速化批处理用引擎)
      - [自定义条目](#自定义条目)
